...
class Dwoo_File{
	...
	public function getCompiledTemplate(Dwoo $dwoo, $compiler = null)
	{
		...
		} elseif ($this->compilationEnforced !== true && $this->isValidCompiledFile($compiledFile)) { //编译文件有效存在
...
/*		echo "bianyi OK";
	...
	protected function getCompiledFilename(Dwoo $dwoo) {
		...
		return $dwoo->getCompileDir() . $fileInfo[0]."_".substr($fileInfo[1], 0,strpos($fileInfo[1], ".")).'_'.C_RELEASE.'.php';
	}
	...
	protected function getCacheFilename(Dwoo $dwoo) {
		...
		$file=$_GET['file'];
		$fileInfo=explode("/", $file);
		return $dwoo->getCacheDir() . $fileInfo[0]."_".substr($fileInfo[1], 0,strpos($fileInfo[1], ".")).'_'.C_RELEASE.'.html';
	}
...
//这是一个统一 调度类  对外接口 里面主要使用了 有关文件操作的 Dwoo_file类、编译相关的Dwoo_Compiler类、抽象插件类。
...
class Dwoo {
	...
	public function get($_tpl, $data = array(), $_compiler = null, $_output = false,$redirect = false) {
		...
		if ($cacheLoaded === true) { //文件已经cache
			...
			//文件未缓存 需要重新缓存 先获取编译文件
			//文件未cache
			if ($doCache === true) {
				$dynamicId = uniqid();
			}

//缓存文件 不存在就去 获取编译文件  获取的过程 可能需要重新编译 可能需要 编译文件有效 直接获取!!
	...
	public function compile(Dwoo $dwoo, $template) {
		...
		while (true) {
			...
			if ($pos === false) { //如果没有找到，跳出循环
				...
				//将找到的左分割符前面的所有数据 也就是不需要编译的 直接存起来!
				//下一次循环 找到左分割符 并且配合上一次解析到的位置$ptr 两个一作差就是我们不需要
				//编译的内容!
				//$ptr的值 真正确定是等本次解析完成之后 此时 $ptr就可以作为上一次解析后的位置
				//结合 新的循环的$pos 两者作差 便得到了要解析的内容!
				...
				//下一步 $ptr 要加的位置 就是+=真正要解析的长度+=右分隔符 这样一次解析完成	

				//配合下一次搜到的 $pos 便得到了不需要解析的内容 再push进去. 解析的内容

				//也应该被 push 了进去 这样全部要解析的内容都被 push 到文件中.

				//处理换行，制表符
				//这里实际上已经是解析开始了!!判断开始解析的首个字符。
				...
				//一定注意 这里的 $ptr 是加上 左结束符的 
				$ptr   += strlen($this->ld);

				$subptr = $ptr;
				//这是真正的编译了 解析到左分割符 进入下面的while循环 
				while (true) {
		...
		$tpl = null;
		//这里的 $output就是 一个解析后的内容 交给 上级函数 将这些编译好的内容写进去!!
		return $output;
	}
...
/*		var_dump($this->curBlock);
		...
		if ($this->curBlock['buffer'] === null && count($this->stack) > 1) {
			...
			//将 当前 block 中 buffer 中的内容 增加$content 也就是 $content加入到 当前 block的 buffer 
	...
	protected function parse($in, $from, $to, $parsingParams = false, $curBlock='', &$pointer = null) {
		...
		$parsed = "";
		//$from 是正文开始正式编译的位置 $pointer是引用传递进来的 用来记录编译完成后的位置
		//便于下一次 循环时 $pos-$pointer 也就是不需要编译的内容 push进去
		//然后是 编译的 内容  $pointer 继续记录 记录至编译结束 然后下一次循环 $pos-该位置是 不需要编译的 以此类推... 
		...
		if ($first==='$') {
			...
			//函数
			//这里 传给 parseFunction的 pointer 也是引用传递.
