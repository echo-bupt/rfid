...
class Page{
     ...
     */
    public function __construct($count,$showPages,$subPages,$href=''){
        $this->count=$count;
        ...
        //如果链接没有设置则获取当前连接
        if(empty($href)){
            $this->href="http://".$_SERVER['HTTP_HOST']."/".$_GET['c']."/".$_GET['m'];
        }else{
            $this->href=$href;
        }
    ...
    protected function pageStyle1(){
        ...
        */
        ...
        $pageStr.='<span>[<a href="'.$this->href.'">首页</a>] </span>';
        //如果当前页不是第一页就显示上页
        if($this->currPage>1){
            $pageStr.='<span>[<a href="'.$this->href.'/page/'.($this->currPage-1).'">上一页</a>] </span>';
        }
 
        foreach ($this->page_arr as $k => $v) {
            if($k==$this->currPage)
            {
                $pageStr.='<span style="color:red;">'.$k.'</span>';
            }else{
        ...
        if($this->currPage<$this->countPages){
            $pageStr.='<span>[<a href="'.$this->href.'/page/'.($this->currPage+1).'">下一页</a>] </span>';
        }
 
        //' " '..'  '..' "  ' 双引号里面可以有多个单引号、、、
        $pageStr.='<span>[<a href="'.$this->href.'/page/'.($this->countPages).'">尾页</a>] </span>';
    ...
    {
        //返回 limit  0,5  
        $start=($this->currPage-1)*($this->subPages);
        $len=$this->subPages;
        return $start.",".$len;
    }
}
